...
const schema = z.object({
  ...
  fromDestination: z.string().min(2, 'From is required'),
  toDestination: z.string().min(2, 'To is required'),
...
export default function BookingFormPage() {
  ...
  const fromDestination = form.watch('fromDestination');
  const toDestination = form.watch('toDestination');

  useEffect(() => {
    const route = buildRoute(fromDestination, toDestination);
    if (route) form.setValue('route', route, { shouldValidate: true });
  }, [fromDestination, toDestination, form]);
  ...
  useEffect(() => {
    ...
    ordersApi.get(orderId).then(({ data }) => {
      ...
      form.reset({
        ...
        fromDestination: o.fromDestination || '',
        toDestination: o.toDestination || '',
  ...
  const onSubmit = async (values) => {
    ...
    try {
      const payload = {
        ...
        route: values.route || buildRoute(values.fromDestination, values.toDestination),
      };
      if (isEdit) {
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="card space-y-4">
        ...
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
          <h3 className="text-sm font-semibold text-slate-900">Travel details</h3>
          <div className="mt-3 grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <input className="input-field uppercase" placeholder="e.g. DAC" {...form.register('fromDestination')} />
              {form.formState.errors.fromDestination && <p className="mt-1 text-xs text-red-600">{form.formState.errors.fromDestination.message}</p>}
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium">To *</label>
              <input className="input-field uppercase" placeholder="e.g. DXB, JED" {...form.register('toDestination')} />
              {form.formState.errors.toDestination && <p className="mt-1 text-xs text-red-600">{form.formState.errors.toDestination.message}</p>}
            </div>
          </div>
        </div>

        <div className="grid gap-4 sm:grid-cols-2">
          <div><label className="mb-1 block text-sm font-medium">Airline / Carrier *</label><input className="input-field" placeholder="e.g. Emirates, Biman" {...form.register('airline')} /></div>
          <div><label className="mb-1 block text-sm font-medium">Route *</label><input className="input-field" {...form.register('route')} readOnly /></div>
          ...
          {journeyType === 'round_trip' && (
            <div>
              ...
              <input type="date" className="input-field" {...form.register('returnDate')} />
              {form.formState.errors.returnDate && <p className="mt-1 text-xs text-red-600">{form.formState.errors.returnDate.message}</p>}
            </div>
          )}
