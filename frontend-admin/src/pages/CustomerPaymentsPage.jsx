...
export default function CustomerPaymentsPage() {
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      setModalOpen(false);
      form.reset({ paymentDate: new Date().toISOString().slice(0, 10), paymentMethod: 'Cash', amount: 0 });
      load();
    } catch (err) {
  ...
  const columns = [
    ...
    { key: 'amount', label: 'Amount', render: (r) => <span className="font-medium text-green-700">{formatCurrency(r.amount)}</span> },
  ...
  return (
    <div className="space-y-4">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Record Customer Payment" wide>
