...
export default function DashboardPage() {
  ...
  const rateLabel = stats?.currentBdtRate ?? brlRate;
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Exchange rate: 1 BRL = ৳ {Number(rateLabel).toFixed(2)} (updated: {rateDate})
          </p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <div className="card">
          ...
          <DualCurrencyAmount
            className="mt-2"
            totalBRL={stats?.totalSpentBRL ?? 0}
            totalBDT={stats?.totalSpentBDT ?? stats?.totalSpent ?? 0}
            size="lg"
          />
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b text-left text-slate-500">
              ...
              <th className="pb-2 pr-4">Total (BRL / BDT)</th>
              ...
              return (
                <tr key={b.id} className="border-b border-slate-100">
                  ...
                  <td className="py-2 pr-4">
                    <DualCurrencyAmount totalBRL={amounts.totalBRL} totalBDT={amounts.totalBDT} size="sm" />
                  </td>
